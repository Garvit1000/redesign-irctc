...
const Feature: React.FC<FeatureProps> = ({ icon, title, description, index, color }) => {
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover-3d group transition-all duration-300 overflow-hidden relative">
      ...
      <div className="relative">
        ...
        <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center mb-5 text-primary group-hover:bg-primary group-hover:text-white transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="h-0.5 w-0 group-hover:w-full mt-4 bg-gradient-to-r transition-all duration-700 rounded-full" 
        style={{ backgroundImage: `linear-gradient(to right, ${color}20, ${color}, ${color}20)` }}></div>
      
      {/* Decorative corner accent */}
      <div className="absolute bottom-0 right-0 w-12 h-12 opacity-0 group-hover:opacity-100 transition-opacity duration-500 overflow-hidden">
        <div className="absolute bottom-0 right-0 w-16 h-16 bg-gradient-to-tl from-primary/10 to-transparent rounded-tl-3xl"></div>
      </div>
    </div>
  );
};
...
const FeaturesSection: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-white to-gray-50 relative overflow-hidden">
      ...
      <div className={bounceAnimation("absolute top-20 left-10 w-8 h-8 bg-primary/10 rounded-full")} style={{animationDelay: "0.5s"}}></div>
      <div className={bounceAnimation("absolute bottom-20 right-10 w-6 h-6 bg-primary/10 rounded-md rotate-45")} style={{animationDelay: "0.8s"}}></div>
      ...
      <div className="container mx-auto px-6 md:px-4 relative z-10">
        ...
        <ScrollAnimation className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
